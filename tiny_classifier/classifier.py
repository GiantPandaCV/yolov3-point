...
def test(model, dataloader: torchvision.datasets.ImageFolder, epoch,
         criterion):
    model.eval()
...
if __name__ == "__main__":
    ...
    for epoch in range(total_epoch):
        ...
        acc=test(model, test_dataloader, epoch, criterion)
        best_acc = max(best_acc, acc)
        scheduler.step()
        if epoch % 10 == 0:
            torch.save(model, "checkpoints/epoch_%d_%.3f.pt" % (epoch, acc))
